...
void consumer_thread_fn(uint8_t *cxl_buf, uint64_t num_iterations) {
	...
	uint64_t *receiver_signal = (uint64_t *) cxl_buf;
...
void sender_thread_fn(uint8_t *cxl_buf, uint64_t num_iterations, uint64_t delay_tsc) {
	...
	// intialize signal
	volatile uint64_t *receiver_signal = (uint64_t *) cxl_buf;
	cxl_buf += HUGE_PAGE_SIZE;
...
int main(int argc, char *argv[]) {
	if (argc != 4) {
		fprintf(stderr, "Usage: %s <0: sender, 1: receiver> <num_iterations> <delay (ns)>\n", argv[0]);
		exit(1);
	}
	bool is_receiver = atoi(argv[1]) == 1;
	uint64_t num_iterations = atoll(argv[2]);
	BUG_ON(num_iterations == 0);
	uint64_t delay_ns = atoll(argv[3]);
	...
	if (is_receiver) {
		consumer_thread_fn(cxl_buf, num_iterations);
	} else {
		sender_thread_fn(cxl_buf, num_iterations, delay_tsc);
	}
